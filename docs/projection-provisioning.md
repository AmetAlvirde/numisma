...
This is live in **`.github/workflows/ci.yml`**. It runs a Postgres **service**
container on the runner and points `NUMISMA_TEST_DATABASE_URL` at it — no
Docker-in-Docker or testcontainers dependency, and (numisma is public) no spend.
So on every push / PR the gated integration tests **run** rather than skip: the
ADR-007 credential invariants and push upsert idempotency are asserted against a
real Postgres, and a provisioning regression fails the build. The shape:

- `services.postgres: postgres:17` with a `pg_isready` health check.
- `env.NUMISMA_TEST_DATABASE_URL: postgres://postgres:postgres@localhost:5432/postgres`
  (the default `postgres` role is a superuser, so the throwaway-DB substrate can
  `CREATE DATABASE` / `CREATE ROLE`).
- steps: checkout → pnpm install `--frozen-lockfile` → `pnpm typecheck` →
...
skips: it scans the built `.vercel/output/static` browser assets and fails if
any server-only token (the pg driver, a `PROJECTION_*` / `AUTH_DATABASE_URL`
connection string, `BETTER_AUTH_SECRET`, or the `composition_snapshot` table
...
independently of `better-auth@1.6.23`; latest is `1.4.21`) and is **checked into
git** (not ignored) — a clean checkout must have it on disk for `auth:apply` to
work. Apply it deterministically and idempotently:

```
pnpm --filter @numisma/web auth:apply       # applies the vendored SQL to AUTH_DATABASE_URL
```

To regenerate the vendored file after a `better-auth` bump, run `auth:generate`
**against an empty database** (the CLI diffs the config against the DB behind
`AUTH_DATABASE_URL`; if that DB already has the auth tables it reports "already
up to date" and writes nothing), then commit the refreshed file:

```
pnpm --filter @numisma/web auth:generate     # writes src/auth/better-auth.schema.sql
git add apps/web/src/auth/better-auth.schema.sql && git commit   # vendor the refreshed schema
```
