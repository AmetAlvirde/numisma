...
| `<dataDir>/head-digest.json`           | Derived, versioned summary of the folded head (the Head Digest) — a breadcrumb that makes a bad-NAV search cheap; schema v2 adds `discardedEventCount`, the number of events the fold read and could not apply (ADR-020, the Discard Channel); never a source of truth (nothing folds it back). | tracked |
