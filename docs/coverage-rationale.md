...
- **`packages/engine/src/events/*.ts`** (91.6% lines, measured) — the covered
  ...
  rejection-message branches** — `events/parse.ts:619-623` (the `funding`
  cash-leg's `reserveId`/`amount` field errors on `PositionAddedTo`, one slice of
  the same "not every malformed-field fixture exists yet" gap as the rest of the
  file) and `events/crossref.ts:824-825,887-888` (the `Transfer`/`PositionOpened`
  cross-reference error-message wrapping) — the validator/cross-reference logic
  runs, but not every individual malformed-field or error-wrapping fixture
  exists yet. `events/fold.ts:649-650` (the lot-selection helper's
  zero-`tierTotal` early return) and `:688-689` (the zero-`totalQuantity` guard
  in `weightedAverageCost`) are the two real fold branches still open; the
  `PriceMarked`-carries-`usdMxn` FX-update branch this bullet used to name is now
  covered.
...
- **`apps/tui/src/event-store.ts`** (the write/ingest path, 87.5% lines,
  uncovered `:249-253,262-266`) — the covered core is the validated ingest
  boundary, dedup, atomic append, archive, and quarantine
  (`event-store.test.ts`). Argv/env parsing (`parseAsOfArg`,
  `parseMagnitudeThresholdArg`) no longer lives here at all — audit finding 35
  extracted it to `spine-args.ts` (100% covered, see below), so this file's
  remaining gap is exclusively `migrateLegacyLog`'s ABORT paths: the
  not-a-migratable-legacy-event throw, the invalid-remigrated-event throw, and
  their siblings — real, reachable failure branches on a one-shot migration
  script, not yet individually itemized here.
...
## 7. Every other measured module below 100% lines — the honesty ledger (audit finding 3)

§§1-6 named specific files because a specific PR or decision put them there.
Between those sections and a fresh `pnpm coverage` run, roughly 25 measured
files sat at less-than-100% with no row anywhere in this document — the repo's
stated "no gap is silent" promise was false for exactly the files a reviewer
would most want it to be true for. This section is the fix: every currently
measured file below 100% lines that §§1-6 do not already name, in one place,
so a `grep` for a module name in this file never comes back empty for a real
gap. Line numbers are the fresh run's; re-run `pnpm coverage` for the current
picture.
...
| --- | --- | --- | --- |
| `record-fill.ts` | 79.76% | `:600-616` | The worst gap in the repo (audit finding 3's headline). `reject(io, "invalid-event"/"rejected-event", …)` — the fill act's own re-parse/re-cross-reference of the transaction it just built, plus the `ordersToAppend` cancellation-sibling assembly. Real rollback-adjacent behavior; `record-fill.test.ts`/`record-fill-reliable.test.ts` drive the refusals earlier in the flow but not this exact pair of late-stage rejects. |
| `import-orders.ts` | 93.75% | `:315-316,473-476` | `unreadable-sidecar-lines` refusal (a partially-read orders book) and one `io.err(...)` skip-line branch in the funding-declaration report join. |
...
| `twelvedata-provider.ts` | 93.1% | `:141-142,176-177` | `observationFromBody`'s rejected-symbol / non-positive-close / unexpected-shape throws, and `barDateFromRow`'s no-usable-date throw. |
...
| `fetch-prices.ts` | 100% lines, 77.5% branch | `:260,286,356,385` | Four per-symbol/FIX `catch` blocks that push a failure record — `runPriceFetch`'s "partial progress always kept" contract; each is a real provider-failure path, exercised in aggregate by `fetch-prices.test.ts` but not with every individual thrown-error shape. |
...
| `push/fixture-synthesis.ts` | 97.28% | `:388-389,661-662` | Two synthesis-edge branches in the fixture generator (test infrastructure adjacent, but the module itself is product code per the `**/*.fixtures.ts` exclude boundary). |
...
| `orders/bitget.ts` | 92.13% | `:420-433` | `total_quantity`/`trigger_price` malformed-cell rejections in the Bitget CSV importer — same shape as every other per-field CSV-row rejection this module already documents elsewhere. |
| `orders/ingest.ts` | 96.55% | `:452-455,487-488` | `:452-455` is a `const _never: never = record` exhaustiveness latch (compile-time only, same defensive pattern as `skip-message.ts`/`orders/select.ts` below — `parseOrderRecord` refuses an unknown `kind` before it can reach here). `:487-488` is the live `timeInForce` difference-detection branch in `detectChangedClaims` — a real comparison arm no current fixture's claim/observation pair exercises. |
| `orders/records.ts` | 84.36% | `:574-579,610-615` | Per-field malformed-value rejections for `orderPlaced`/`orderFilled` records (`timeInForce`, `triggerPrice`, `filledQuantity`) — the same per-field pattern as `events/parse.ts`. |
...
| `price-feed/mark.ts` | 90.47% | `:132-133,137-138` | `parseMarkTime`'s malformed-format and out-of-range-hour/minute throws — defensive, `DEFAULT_CONFIG.markTime` and any override are validated shapes, not raw operator input. |
...
| `orders.ts` (preferences package) | 89.74% | `:222-230` | The lock-contention retry path in the sidecar's advisory file lock (`open(lockPath, "wx")` racing another writer) — a real concurrency branch, exercised only under contention. |
...
measured file below 100% lines that the earlier sections do not individually
