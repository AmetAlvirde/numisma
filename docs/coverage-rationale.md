...
| `apps/web/src/push/push.ts` | Self-executing `tsx` script (`pnpm push` / `db:init`): top-level `main().then(..., process.exit)` that loads the fixture and upserts it. Same script category as `apps/tui/src/report.ts` / `spine.ts` — importing it runs `main()`, so there is no unit to assert as written. | The shared derivation it depends on (`fundIdOf`, `COMPOSITION_SNAPSHOT_SCHEMA_VERSION`) is unit-tested via `contract.test.ts`; the DDL it applies is the tested `readSchemaDdl()` from `provision.ts`; slice #127 adds the DB-backed upsert-idempotency test and, if it extracts a pure derivation, that unit moves back under the number. |
