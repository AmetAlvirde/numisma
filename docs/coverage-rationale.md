...
script — now a thin argv/credential wrapper over the measured `push-core.ts`),
`lib/auth.ts` / `lib/auth-client.ts` / `lib/query.ts` / `routes/api/auth/$.ts`
(framework wiring with module-level side effects, not Node-unit-testable), and
`routeTree.gen.ts` (generated). Each would otherwise be dishonest 0%.
