...
| --- | --- | --- | --- | --- |
| `packages/components` | `@numisma/components` | Any TSX-transforming bundler | The shared React component layer, shipped as **unbuilt TSX** with no `dist`, no build script and no CSS of its own, plus the `--nms-*` token specification whose values each consumer supplies ([ADR-023](../context/adr/ADR-023-unbuilt-tsx-and-a-namespaced-token-spec-the-consumer-supplies.md)). Upstream shadcn components enter only through `pnpm components:add`; house components are migrated in from `apps/web` by hand. | [`component-package.md`](./component-package.md), then `src/tokens.ts` (the spec, with the argument in its header) |
