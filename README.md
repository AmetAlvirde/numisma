...
eight members are versioned together at the monorepo version.

| Package                                                    | Runtime                                    | Owns                                                                                                                                                                                                                                                                                       |
| ---------------------------------------------------------- | ------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| [`@numisma/components`](./docs/component-package.md)       | Any TSX-transforming bundler; ships no build output | The shared React component layer, shipped as **unbuilt TSX** with no `dist` and no CSS of its own, plus the `--nms-*` token specification whose values each consumer supplies ([ADR-023](./context/adr/ADR-023-unbuilt-tsx-and-a-namespaced-token-spec-the-consumer-supplies.md)). Upstream shadcn components enter through `pnpm components:add`, house components by hand from `apps/web`, and every one of them leaves through the hand-curated `src/index.ts`. |
...
| [`@numisma/workbench`](./docs/component-package.md)        | Vite + react-cosmos, no SSR                | The component workbench ([ADR-024](./context/adr/ADR-024-react-cosmos-as-the-workbench-standalone.md)): every state of every package component, under three theme modes. It is also the second token consumer, and its no-SSR build is what makes a client/SSR fault separable. It imports no module from `apps/web`. |
...
| [`docs/component-package.md`](./docs/component-package.md)                 | What the component package ships, what a consumer owes it, the two silent styling failures, and the manual theming pass. |
