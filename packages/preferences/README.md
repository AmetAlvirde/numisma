...
| `resolvePlansPath` | function | Resolve `<dataDir>/plans.jsonl` under the same ADR-006 invariant — absolute and homedir-derived, never CWD-relative. The three cases (unset/blank → the accumulus default, absolute → verbatim, relative → a loud throw) live once in `sidecar-io.ts`'s `resolveSidecarPath`. |
