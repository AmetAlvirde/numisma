...
export class PositionRepository {
   ...
   */
  async findByLifecycle(lifecycle: string, userId: string): Promise<Position[]> {
    try {
  ...
  async create(position: Omit<Position, 'id'>): Promise<Position> {
    ...
    try {
      ...
      if (position.positionDetails.orders && position.positionDetails.orders.length > 0) {
   ...
   */
  async update(id: string, data: Partial<Omit<Position, 'id'>>): Promise<Position> {
    // Validate with Zod schema
      ...
      if (data.lifecycle && data.lifecycle !== currentPosition.lifecycle) {
        await this.prisma.lifecycleHistory.create({
          data: {
            ...
            notes: data.lifecycleNotes || `Updated lifecycle from ${currentPosition.lifecycle} to ${data.lifecycle}`
          }
        });
      }
      
      // Handle capital tier change if needed
      if (data.capitalTier && data.capitalTier !== currentPosition.capitalTier) {
        await this.prisma.capitalTierHistory.create({
          data: {
            ...
            notes: data.capitalTierNotes || `Updated capital tier from ${currentPosition.capitalTier} to ${data.capitalTier}`
          }
        });
      }
      ...
      if (data.positionDetails) {
        await this.prisma.positionDetails.update({
          ...
          data: {
            ...
            dateOpened: data.positionDetails.dateOpened ? dateOrGenesisToDate(data.positionDetails.dateOpened) : undefined,
   ...
   */
  async addJournalEntry(positionId: string, entry: {
    thought: string;
    attachments?: string[];
    userId: string;
  }): Promise<void> {
   ...
   */
  async updateMetrics(positionId: string, metrics: {
    realizedPnL: number;
   ...
   */
  private mapToDomainModel(position: PrismaPosition & {
    market?: any;
    walletLocation?: any;
    positionDetails?: any;
    metrics?: any;
    ...
    const positionDetails = position.positionDetails ? {
      ...
      dateOpened: databaseDateToDateOrGenesis(position.positionDetails.dateOpened),
    ...
    return {
      ...
      lifecycleHistory,
      capitalTierHistory,
      userId: position.walletLocation?.userId // Added for convenience
    };
  }
}
