...
/** Every custom property read via `var(--…)` in a file's text. */
function varReads(text: string): string[] {
  return [...text.matchAll(/var\(\s*(--[\w-]+)/g)].map((match) => match[1]!);
}
...
describe("the --nms-* token spec", () => {
  ...
  it.each(files.map((path) => [path.slice(SRC.length), path] as const))(
    ...
    (_label, path) => {
      const stray = varReads(readFileSync(path, "utf8")).filter(
  ...
  it.each(files.map((path) => [path.slice(SRC.length), path] as const))(
    ...
    (_label, path) => {
      const undeclared = varReads(readFileSync(path, "utf8")).filter(
