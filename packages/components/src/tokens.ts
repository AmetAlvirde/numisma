...
export const NMS_TOKENS = [
  ...
  {
    name: "--nms-ok",
    value: "oklch(0.43 0 0)",
    note: "The all-clear FILL — a badge asserting that the fold excluded nothing and the marks arrived. `SummaryCard`'s data-safety badge, green arm. A fill rather than a type colour, which is what makes it the loudest thing in themed mode when nothing repaints. DARKENED TWICE, for two different rules. Spec #451 S3 moved it off 0.62, where white on it reached 3.64:1 and SC 1.4.3 wants 4.5 — the badge's label is what is being read, so the fill is what moves. That landed it at 0.46, one hundredth of L from `--nms-destructive` and `--nms-neg` at 0.45, and this row recorded the collision and said nothing caught it. Something does now: `app-token-drift.test.ts` guards the grayscale STATE FAMILY at 0.02 L, the spacing every other gap in that family already keeps, and it went red on this token. 0.43 is the move, DARKER rather than lighter because this token is only ever a fill under white and darker only raises that ratio — 7.13:1 at the old value, 8.11:1 here, so the pair it was moved for in the first place cannot break. Nothing in the package or the app reads it as type: `summary-card.tsx` is its one render and it is `bg-[var(--nms-ok)] text-white`.",
  },
  {
    name: "--nms-warn",
