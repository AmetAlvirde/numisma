...
export const EXCLUDED_CONTRAST_PAIRS: readonly ExcludedPair[] = [
  ...
  },
  {
    ...
    surface: "--nms-card",
    reason:
      "Button `variant=\"link\"`, which paints `text-primary` with no fill. It is the same one-token-two-jobs shape `--nms-warn` had before this list existed: `apps/web` resolves `--nms-primary` to an accent chosen as a FILL under white, and no single blue clears 4.5:1 both as that fill and as type on a dark card. It measures 3.77:1 as type on the app's card. EXCLUDED ON A CONDITION, AND THE CONDITION IS ENFORCED: the variant renders in `apps/workbench`'s button fixture and on no shipped app surface, so nothing is failing for a user today, and `ops/components/link-variant.test.ts` is what reds the day that stops being true — it scans every app in the workspace's `apps` group except the declared review surfaces. Nothing else in this list rests on a fact that a single commit elsewhere can falsify, which is why this is the one entry with a guard named in it rather than an argument alone. IF THAT GUARD GOES RED, THE ANSWER IS A MINT, not a widened excuse: the variant needs a text-legible colour before it ships, that colour is paired here, and this exclusion goes. Asserting it TODAY would demand the same mint with no case asking for one, and spec #451 §10 puts the value decision with the first case that needs it.",
  },
  {
    foreground: "--nms-warn",
    surface: "--nms-card",
