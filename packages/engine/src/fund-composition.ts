// Domain types, read models, parse-result/warning unions, and the assurance
// vocabulary now live in ./contracts.js. They are re-exported here verbatim so
// the public type surface (reached via index.ts) is unchanged — exports only
// moved, none were added or removed.
export type {
  Currency,
  ExecutionMode,
  Direction,
...
// Local bindings for the contracts this module references internally.
// `NamedRecord` and `CapitalRecordBase` are package-internal — imported here but
// intentionally not re-exported above, so they stay off the public surface.
import type {
  Currency,
  ExecutionMode,
  Direction,
