...
| `buildEventReference`, `crossReferenceEvent`                                                                      | functions | The ingest cross-reference: `buildEventReference` PROJECTS `foldEvents(genesis, acceptedSoFar)` into the known ids, Reserve balances, open lots and last Closes the gate reads, and `crossReferenceEvent` rejects unknown-id / colliding-id / insufficient-debit / implausible-magnitude events against it. ONE world-state — the gate reads the fold rather than shadowing it, so it cannot be wrong about the world the fold will produce (ADR-015, audit findings 31 / 1). To advance across a batch, rebuild from the accepted prefix; there is no in-place advance and `applyEventToReference` no longer exists. |
