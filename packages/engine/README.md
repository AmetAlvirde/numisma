...
| `walkPendingInbox`                                                                                                | function  | The ONE pending-inbox walk both ingest paths fold with: parse each candidate, dedup-skip by id BEFORE the guard, cross-reference the rest, advance the reference on accept. Halts on an invalid candidate; halts or collects on a rejection, per the caller's own error policy. |
...
| `events/ingest-walk.ts` | `IngestWalkResult`, `IngestWalkRejection`, `IngestWalkInvalid`, `IngestWalkOptions`, `walkPendingInbox` | The ONE pending-inbox walk (audit finding 9). Folds an ordered batch of untrusted candidates onto an `EventReference`: `parseEvent`, dedup by stable `id` before the guard, `crossReferenceEvent`, `applyEventToReference` on accept. Both the TUI spine's `ingestInbox` and `@numisma/price-feed`'s fetch-time rejection pre-check consume it — the pre-check's whole value is byte-fidelity to the spine, and `apps/price-feed` cannot import `apps/tui`. Error POLICY stays with each caller (halt-and-throw vs collect-and-report), which is exactly where they deliberately differ. IO-free: candidates in as data, verdicts out as data. |
