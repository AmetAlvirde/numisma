...
> leg); `fold.test.ts`, `event-ingest.test.ts`, `events/one-world-state.test.ts` and `events/ingest-walk.test.ts` (the event-sourcing spine), with `position-born-by.test.ts` and `position-seal.test.ts` covering the ingest gate's date-ordering rules;
