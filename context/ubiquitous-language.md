...
| **Fold**                    | The pure derivation of state from history: Genesis plus the Durable Log's events, applied in `asOf` order and then log order so same-day events resolve by the order they were appended, producing a `FoldedReview` — `{data, skipped}`, a `FundReviewData` read model as of a point in time plus every event the fold read and could not apply (`foldEvents`, `packages/engine/src/events/fold.ts`). It is the **one** canonical derivation — `buildCompositionReport` and the whole dashboard consume `data` unchanged, and since ADR-015 the ingest gate projects that output rather than mirroring its transitions, so a change to the Fold moves what the gate admits and not only what the dashboard shows. The result shares no mutable state with Genesis. See ADR-003, ADR-015, and ADR-020 for `skipped`, the Discard Channel — and ADR-021 for the rule that every discard is total, so an event the Fold could not fully apply moves nothing at all. | reduce, projection, rebuild |
| **Discard**                 | One event the Fold read and then **applied nothing at all** from. Not a rejection and not a refusal: the event is already in the immutable log, the Fold does not throw on it, and the ingest gate carries it through without acting on it. The record is `{eventId, index, verb, reason, detail}` — located by event id rather than line number, because the log is append-only, and its `detail` is fixed prose that never quotes event content and never carries a fund figure, since these lines surface daily on channels whose value is staying stable and quotable. See ADR-020. | rejection, error, skip-the-leg |
| **Discard Channel**         | The Fold's `skipped[]` and the surfaces that carry it: the ingest gate's envelope, `discardedEventCount` on the Head Digest, the unattended one-line verdict, and the interactive enumeration. Every counting surface goes through one dedup key — (`eventId`, `reason`) — so no two of them can report different figures for one log. Its rule is **report, never refuse**. See ADR-020. | warning channel, error log |
| **Discard Reason**          | Why the Fold could not apply an event, from a **closed** three-member vocabulary: `position-absent` (the verb named a position the Fold has no record of), `reserve-absent` (a cash leg named a reserve it has no record of), `provenance-absent` (a lot-derived cash leg had no lots to inherit cost-basis attribution from). **A reason names WHAT WAS MISSING and never how much of the event survived the miss** — every member means total discard, and a member added later inherits that rather than choosing it. A `Transfer` applies both legs or neither and reports one record for the event. See ADR-021. | partial apply, dropped leg |
| **Head Digest**             | A derived, versioned summary of the folded head — `fundValueUsd` from the canonical composition report, open/closed Position counts, head event id, the writing app version, and (schema v2) `discardedEventCount`, the number of distinct events the fold read and could not apply (the channel's own dedup key, so the digest and the unattended fold line report the same figure for one log). Re-derivable, committed alongside the durable log on each ingest (`head-digest.json`), overwritten in place, and never a source of truth (no engine reader folds it back). See ADR-006, the ADR-003 Head Digest amendment, and ADR-020 (the Discard Channel) for `discardedEventCount`. | checkpoint, snapshot    |
| **Log History**             | The private sibling git repository (`~/Dev/<fund>`) that versions the durable log — `genesis.json`, `events.jsonl`, the Head Digest, and every Sidecar (`preferences.jsonl`, `orders.jsonl`, `plans.jsonl`, `reconciliations.jsonl`) — giving the git-ignored log a restorable, provenance-stamped history; discovered via `NUMISMA_DATA_DIR`. See ADR-006 for the tracked-file allowlist, which each Sidecar joins in turn. | backup, data folder     |
...
| **Sidecar**             | A durable, append-only, git-versioned file beside the event log, carrying its own on-load validation contract, joined to the Fold at read time by a pure selector, and **never folded**. Four members: `preferences.jsonl`, `orders.jsonl`, `plans.jsonl`, `reconciliations.jsonl`. A Sidecar has every one of the Durable Log's disciplines and none of its authority — what makes the log the log is that `foldEvents` reads it and NAV comes out. See ADR-004. | config file, settings, event log |
| **Plan**                | The operator's durable DECLARATION OF INTENT for one Position, recorded in `plans.jsonl` and hand-authored: a `dcaLadder`, a `dcaTime` cadence (`daily`/`weekly`/`monthly`), or the `noPlan` terminator. Identity is `positionId` + `effectiveAt`; there is no edit and no delete, so a later `effectiveAt` for the same Position supersedes the earlier one and `noPlan` is the only way to stop one. A Plan naming a Position that does not exist yet is **legal** — naming the Position a ladder is meant to become is the fact being authored. Nothing a Plan declares can move NAV. | strategy, order, schedule |
| **Rung**                | One declared price level of a `dcaLadder` Plan. No two Rungs in a ladder share a price, and a ladder carries its own id, so an imported Order can name the Rung it was placed for and the operator can ratify that pick. | order, level, tranche |
| **Fill Path**           | The read-time reconciliation of one declared ladder against the whole order stream and the recorded Lots: per Rung, what the venue SHOWED and what the book DID, as two axes that are never collapsed into one enum, plus the orphan Lots no Rung claims. Descriptive; it writes nothing. | fill report, ladder status |
| **Reconciliation Trail** | The machine-written `reconciliations.jsonl` record of **what the operator was told**: at a named moment a reader compared one fill against the Plan in force and showed a verdict. It is never a record of what is true — `plans.jsonl` stays authoritative, and a trail line never overrides it or is read in preference to it. Each line carries a denormalized copy of the declared values as shown, because Plan supersession is append-only and the verdict as shown is not recoverable later. A line that disagrees with a fresh re-derivation is a **finding, not a corruption**. | audit log, plan state |
...
- A **Position** belongs to exactly one **Fund**, one **Portfolio**, one
  ...
  **Account**.
- A **Position** contains one or more **Lots**; a **Reserve** contains zero or
  more. An untiered Position is written as one `c1` **Lot**, not as a flat
  amount; untraceable cash stays untiered rather than being forced into a false
  `c1`, which is why the tier total deliberately sits below 100% of the fund.
- A **Lot** preserves **Capital Tier** attribution.
- A **Plan** names exactly one **Position**, which need not exist yet.
- A **Reconciliation Trail** line reports on exactly one **Fill** against the
  **Plan** in force, and never overrides that **Plan**.
