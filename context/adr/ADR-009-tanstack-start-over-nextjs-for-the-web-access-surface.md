...
_(Two route names in the measurement above were wrong when written and are wrong
now: there has never been a `/dashboard` or an `/api/health` in this app. The
finding holds in shape, shell 200s and DB-dependent redirects with no 500s, but
take the route table from `apps/web/src/routeTree.gen.ts` and the corrected
per-route results from `docs/web-deploy-runbook.md`, not from this paragraph.
Two more routes have since landed, `/ladder/$planId` and the dev-only
`/ladder-fixture/$state`.)_
