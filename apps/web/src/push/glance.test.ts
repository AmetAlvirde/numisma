...
import { buildGlanceBlock, summarizeVenueDark } from "./glance.ts";
...
describe("the quiet Sunday (the case V1 exists to protect)", () => {
  ...
  it("agrees with a live fold of the real log, when the log is present", async ({
    ...
    //
    // The venue-dark verdict (#266) is RE-DERIVED live from the same log, not copied
    // out of the recording: `buildGlanceBlock` is pure and takes it as an argument
    // (like the Reserve floor beside it), so feeding the recorded value back in would
    // make this half of the equality vacuous. `summarizeVenueDark` needs the raw event
    // list, which is the push shell's read rather than the builder's — so the read
    // happens here and the agreement is real.
    const log = await loadEventLog(resolveEventStorePaths().log);
    const venueDark = summarizeVenueDark(log.events, QUIET_SUNDAY);
    expect(buildGlanceBlock(data, report, 10, venueDark)).toEqual(recorded.glance);
    // …and the recording must CARRY the field, so a fixture regenerated by a build
    // whose derivation silently degraded cannot pass this off as agreement.
    expect(recorded.glance.venueDark).toBeDefined();
