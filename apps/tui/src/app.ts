...
// genesis + log to `--as-of` (or current state), and `r` reloads = re-fold. The
// ingest report and the liveness lines are written to stderr BEFORE the alternate
// screen takes over — deliberately, so they reach you without an in-TUI banner to
// go looking for. The startup data path itself lives in `prepareStartup` (a
// tested seam shared with the openTUI verification harness); this file owns only
// the renderer wiring.
...
async function loadOpenTuiCore(): Promise<typeof import("@opentui/core")> {
  try {
    ...
    const detail = error instanceof Error ? error.message : String(error);
    throw new Error(
      `openTUI could not load in this runtime. @opentui/core currently needs Bun or a Node build that exposes node:ffi. This workspace uses Bun only to run openTUI while keeping pnpm for package management. Runtime: ${process.release.name} ${process.version}. Original error: ${detail}`,
    );
  }
}
